(() => {
  ...
  // лёгкий < сложный по макс X (как было)
  ...
  // УСЛОЖНЕНИЕ: сколько зон перекрывает вратарь (логика сейва)
  // было 2; теперь: лёгкий=3 (~1.5x), сложный=4 (~2x)
  const COVER_COUNT = {
    easy: 3,
    hard: 4,
  };
  ...
  const state = {
    ...
    // визуальная позиция перчаток (2 клетки)
    goalieCells: [0, 1],
    // логическое перекрытие (3/4 клетки) — именно это используется для сейва
    goalieCover: [0, 1, 2],
  ...
  function updateTexts() {
    ...
    diffHint.textContent = state.diff === "hard"
      ? "Сложный: вратарь перекрывает больше зон и двигается чаще."
      : "Лёгкий: вратарь перекрывает больше зон, но двигается мягче.";
  ...
  // УСЛОЖНЕНИЕ: расширяем перекрытие до 3/4 клеток, но перчатки (визуал) остаются парой
  function expandCoverageFromPair(pair, desiredCount) {
    const set = new Set(pair);
    if (desiredCount <= 2) return [...set];

    // собираем кандидатов вокруг пары
    let frontier = [...pair];
    let tries = 0;

    while (set.size < desiredCount && tries < 50) {
      tries++;
      const pick = frontier[rngInt(frontier.length)];
      const nbs = neighbors(pick);

      // перемешаем соседей чуть-чуть
      ...
      if (!added && frontier.length > 2) frontier.pop();
    }
  ...
  function updateGoalieState(pair) {
    ...
    const want = state.diff === "hard" ? COVER_COUNT.hard : COVER_COUNT.easy;
    state.goalieCover = expandCoverageFromPair(pair, want);
  ...
  function startGoalie() {
    ...
    updateGoalieState(randomAdjacentPair());
    ...
    state.goalieTimer = setInterval(() => {
      ...
      // небольшой "смарт" в сложном — чаще тянется к центру (как было)
      if (state.diff === "hard") {
        if (rngInt(100) < 45) {
          const central = [6, 7, 8];
          const base = central[rngInt(central.length)];
          const opts = neighbors(base);
  ...
  async function onShoot(idx) {
    ...
    // ВАЖНО: сейв теперь проверяется по расширенному перекрытию (3/4 клеток)
